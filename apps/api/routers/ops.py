...
async def services():
    ...
    # Defect/vision extraction stays on Gemini even in AWS mode (Rekognition + Bedrock
    # Haiku cover serial OCR + disposition prose; only "bedrock" uses Claude vision).
    vision_engine = {
        "gemini": "Google Gemini Flash (vision, free tier)",
        "aws": "Google Gemini Flash (vision, free tier)",
        "anthropic": "Claude Sonnet (vision)",
        "bedrock": "Claude Sonnet on Bedrock (vision)",
        "mock": "cached SKU results (offline)",
    }.get(mode, mode)

    # AWS mode swaps in real Amazon services for serial OCR, grading corroboration,
    # and the disposition narration; other modes render exactly as before.
    serial_engine = "Amazon Rekognition DetectText" if is_aws else vision_engine
    grading_engine = (
        "deterministic rubric + Rekognition labels (corroboration)"
        if is_aws else "deterministic rubric"
    )
    disposition_engine = (
        "rules + Bedrock Claude Haiku" if is_aws else "rules + AI explanation"
    )
    ...
    return {
        ...
        "services": [
            ...
            {"name": "Serial OCR", "engine": serial_engine, "status": "REAL"},
            {"name": "Fingerprint Identity", "engine": "local CLIP-style embedding + pHash + serial", "status": "REAL"},
            {"name": "Condition Grading", "engine": grading_engine, "status": "REAL"},
            {"name": "Trust / Fraud Score", "engine": "composite + behavioral flags", "status": "REAL"},
            {"name": "Disposition Routing", "engine": disposition_engine, "status": "REAL"},
