...
async def verify(session_id: str, fraud_mode: bool = Form(False)):
    ...
    # The serial presented at return time. Fraud demo presents the swap unit's serial.
...
async def grade_session(session_id: str, fraud_mode: bool = Form(False)):
    ...
    grade_result: GradeResult = grading_svc.grade(extraction)
...
async def disposition(session_id: str):
    ...
    bus.emit(
        ...
        actor="system",
        detail={"path": result.path.value, "explanation": result.explanation},
    )
    return {
